...
workbench:
  ...
  codeRefs: []
  pathHistory:
    - "C:/overview/workbench-spec.md"
  path: /overview/workbench-spec.md
owner: platform
status: active
updated: 2025-12-27
---
...
- repo scaffolding for semantic document roots, specs, architecture, and work
...
- execution (/work)
- design/decisions (/overview, /contracts, /decisions, /runbooks, /tracking)
...
- ADR: architecture decision record, stored in /decisions.
...
- Doc templates are copied into /templates.
...
### Template tokens
Templates in `templates` and `work/templates` are copied verbatim,
...
Docs use YAML front matter aligned with `/schemas/doc.schema.json`. Workbench keeps path metadata so moved files can be detected and links updated.
...
`/schemas/workbench-config.schema.json`
...
For a step-by-step CLI workflow, see [runbooks/spec-cli-workflow.md](../runbooks/spec-cli-workflow.md).
...
- ADRs in `/decisions` must include a "Related specs" section with at least one link to a spec in `/specs` or `/architecture`.
...
### JSON
Stable schema per command, documented in contracts/commands.md.
