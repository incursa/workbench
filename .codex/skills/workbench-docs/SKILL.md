...
Create a doc:
```bash
workbench.ps1 doc new --type doc --title "Title" --path overview/title.md --work-item TASK-0001
```

Create a spec:
```bash
workbench.ps1 doc new --type spec --title "Title" --path overview/title.md --work-item TASK-0001
```

Create an architecture doc:
```bash
workbench.ps1 doc new --type doc --title "System design" --path architecture/system-design.md --work-item TASK-0001
```

Create an ADR:
```bash
workbench.ps1 doc new --type adr --title "Decision" --path decisions/ADR-YYYY-MM-DD-title.md --work-item TASK-0001
```

Link a doc to work items:
```bash
workbench.ps1 doc link --type spec --path overview/title.md --work-item TASK-0001
```

Unlink a doc from work items:
```bash
workbench.ps1 doc unlink --type adr --path decisions/ADR-YYYY-MM-DD-title.md --work-item TASK-0001
```
