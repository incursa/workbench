...
## REQ-CLI-DOC-SHOW-0004 Missing-target handling

`doc show` MUST fail clearly when the referenced document cannot be resolved.

## REQ-CLI-DOC-SHOW-0005 Output identity

`doc show` MUST include the resolved document path in its rendered output.

## REQ-CLI-DOC-SHOW-0006 Metadata completeness

`doc show` MUST render the full document metadata block before the body.

## REQ-CLI-DOC-SHOW-0007 Stable field order

`doc show` MUST keep rendered metadata fields in a stable order.
