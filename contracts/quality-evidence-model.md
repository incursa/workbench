...
workbench:
  ...
  pathHistory:
    - "C:/contracts/quality-evidence-model.md"
  path: /contracts/quality-evidence-model.md
owner: platform
status: draft
updated: 2026-03-07
---
...
- authored truth is human-owned and lives under the semantic documentation roots (`overview/`, `contracts/`, `decisions/`, `runbooks/`, `tracking/`, `templates/`, and `specs/`)
...
| --- | --- | --- | --- |
| Authored testing intent | `contracts/test-gate.contract.yaml` | human-authored | Compatibility-preserving V1 source for expectations, risk notes, confidence targets, and intentional gaps |
...
V1 should extend the existing `contracts/test-gate.contract.yaml`
...
and follows the schemas in `schemas/`.
...
| File | Purpose |
| --- | --- |
| `schemas/test-inventory.schema.json` | Discovered tests and test projects |
| `schemas/test-run-summary.schema.json` | Latest normalized run results |
| `schemas/coverage-summary.schema.json` | Latest normalized coverage results |
| `schemas/quality-report.schema.json` | Compared view of authored truth versus observed truth |
...
- `contracts/test-gate.contract.yaml` becomes the authored starting
  point rather than a standalone gate definition.
- `contracts/test-matrix.md` remains a curated mapping doc and can later
